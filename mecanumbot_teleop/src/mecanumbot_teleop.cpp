...
{
    // Setup the parameters (default config for xbox-one controller)
    ...
    declare_parameter<double>("turn.left.deadzone", 1.0);
    declare_parameter<double>("turn.left.scale", 0.0);
...
double MecanumbotTeleop::get_axis_value(std::unique_ptr<sensor_msgs::msg::Joy> & msg, AxisConfig& config)
{
    if (msg->axes.size() >= (size_t)config.Axis) {
        double value = (double)msg->axes[config.Axis] * config.Scale;
